...
export class CartComponent implements OnInit{
  ...
  constructor(private cartService: CartService, private modalService: MdbModalService) {
    ...
    this.products.forEach((product) => {
      const cartItem: CartItem = {
        ...
        finalPrice: product.unitPrice - (product.unitPrice * product.discount / 100),
      };
  ...
  calculateTotalPrice(): void {
    this.totalPrice = this.cartItems.reduce((acc, item) => acc + (item.finalPrice || 0), 0);
  }
